...
public:
    ...
    void set_roi_target(const struct Location &target_loc, Vector3f roi_target_velocity = Vector3f()) { set_roi_target(_primary,target_loc, roi_target_velocity); }
    void set_roi_target(uint8_t instance, const struct Location &target_loc, Vector3f roi_target_velocity = Vector3f());
