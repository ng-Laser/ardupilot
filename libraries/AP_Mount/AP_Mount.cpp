...
void AP_Mount::set_roi_target(uint8_t instance, const struct Location &target_loc, Vector3f roiVelocity)
{
    // call instance's set_roi_cmd
    if (instance < AP_MOUNT_MAX_INSTANCES && _backends[instance] != NULL) {
        _backends[instance]->set_roi_target(target_loc, roiVelocity);
    }
}
